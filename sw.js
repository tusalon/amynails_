...
const CACHE_NAME = 'amynails_-v12';
const urlsToCache = [
  '/amynails_/',
      ...
      return Promise.all(
        cacheNames.map(cacheName => {
          if (cacheName !== CACHE_NAME) {
            console.log('Yi  🗑️ Eliminando cache antiguo:', cacheName);
...
self.addEventListener('fetch', event => {
  ...
  // as ⚠️ NO INTERCEPTAR WHATSAPP (ESENCIAL PARA iOS)
...
self.addEventListener('message', event => {
  ...
  if (event.data && event.data.type === 'SKIP_WAITING') {
    console.log('a ⏩ Saltando waiting...');
    self.skipWaiting();
  }
  
  if (event.data && event.data.type === 'CLEAR_CACHE') {
    console.log('Y1 🧹 Limpiando todo el cache...');
    caches.keys().then(cacheNames => {
      cacheNames.forEach(cacheName => {
        caches.delete(cacheName);
        console.log('Yi  🗑️ Cache eliminado:', cacheName);
      });
    });
  }
});
